...
#ifdef USE_FLOAT64
  #warning "Compiling the float64 version!"
  #define FLOAT_TYPE_TO_USE double
#else
  #warning "Compiling the float32 version!"
...
void mat_set(Matrix* Mat,int l,FLOAT_TYPE_TO_USE z);
void mat_set_init(Matrix* Mat);
FLOAT_TYPE_TO_USE jacobi(int n,Matrix* M1,Matrix* M2,Matrix* M3,
             Matrix* M4,Matrix* M5,Matrix* M6,Matrix* M7);

FLOAT_TYPE_TO_USE   omega=0.8;
...
newMat(Matrix* Mat, int mnums,int mrows, int mcols, int mdeps)
{
  ...
  Mat->mdeps= mdeps;
  Mat->m= NULL;
  Mat->m= (FLOAT_TYPE_TO_USE*) 
    malloc(mnums * mrows * mcols * mdeps * sizeof(FLOAT_TYPE_TO_USE));
...
void
mat_set(Matrix* Mat, int l, FLOAT_TYPE_TO_USE val)
{
  int i,j,k;
...
mat_set_init(Matrix* Mat)
{
  ...
  for(i=0; i<Mat->mrows; i++)
    for(j=0; j<Mat->mcols; j++)
      for(k=0; k<Mat->mdeps; k++)
        MR(Mat,0,i,j,k)= (FLOAT_TYPE_TO_USE)(i*i)
          /(FLOAT_TYPE_TO_USE)((Mat->mrows - 1)*(Mat->mrows - 1));
}
